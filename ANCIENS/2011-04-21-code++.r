...
for (iteration in 1:repetition){
	...
	# f_dist_mue <- runif(nf, min=0, max=f_max_dist_mue)
	# f_date_receptivite <- floor(f_max_dist_mue/2)
	f_date_receptivite <- f_max_dist_mue/2
	f_age <- numeric(nf)
	f_nb_mue <- numeric(nf)
...
# m_dist_mue <- runif(nm, min=0, max=m_max_dist_mue)
	m_age <- numeric(nm)
	m_nb_mue <- numeric(nm)
	...
	for (tps in 1:tps_max) {
	  ...
	  # pour chaque mort, un individu nait :
		...
		f_date_receptivite[no_femelle_mort] <- f_max_dist_mue[no_femelle_mort]/2
		...
		if (nbmuetoday > 0) {
			...
			f_date_receptivite[femelle_en_mue] <- f_dist_mue[femelle_en_mue]/2
		...
		prop_receptive[tps] <- (nb_receptive[tps] * 100) / nf
