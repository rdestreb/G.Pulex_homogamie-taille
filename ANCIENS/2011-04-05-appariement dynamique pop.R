...
#     if (nf <= nm){
        ...
        if (nblibretoday > 0 & nbcelibataire > 0){
          ...
          # sélection aléatoire parmis les mâles libres :

            if (nblibretoday <= nbcelibataire) {
                if (nbcelibataire == 1) {
					chanseuse <- sample(no_femelle_libre,nbcelibataire)
                    no_male_selec <- celibataire 
                }else{
                    no_male_selec <- sample(celibataire, nblibretoday)
                }
            }
          if (nblibretoday > nbcelibataire) {
			chanseuse <- sample(no_femelle_libre,nbcelibataire,replace=FALSE)
			no_male_selec <- sample(celibataire, nbcelibataire)
          }
            
            femelle$no_male[chanseuse] <- no_male_selec
            femelle$taille_male[chanseuse] <- male$taille[no_male_selec]
            femelle$dist_mue_male[chanseuse] <- male$dist_mue[no_male_selec]
            
            male$no_femelle[no_male_selec] <- chanseuse
            male$taille_femelle[no_male_selec] <- femelle$taille[chanseuse]
            male$dist_mue_femelle[no_male_selec] <- femelle$dist_mue[chanseuse]
...
#     }else{
#       
#       nblibretoday <- length(male$no_femelle[male$no_femelle == 0])
#       
#       if (nblibretoday > 0){
#           
#           no_femelle_selec <- sample(femelle$no[femelle$no_male==0],nblibretoday)
#           no_male_libre <- male$no[male$no_femelle == 0]
#           
#           male$no_femelle[no_male_libre] <- no_femelle_selec
#           male$taille_femelle[no_male_libre] <- femelle$taille[no_femelle_selec]
#           male$dist_mue_femelle[no_male_libre] <- femelle$dist_mue[no_femelle_selec]
#           
#           femelle$no_male[no_femelle_selec] <- no_male_libre
#           femelle$taille_male[no_femelle_selec] <- male$taille[no_male_libre]
#           femelle$dist_mue_male[no_femelle_selec] <- male$dist_mue[no_male_libre]
#       }
#     }
