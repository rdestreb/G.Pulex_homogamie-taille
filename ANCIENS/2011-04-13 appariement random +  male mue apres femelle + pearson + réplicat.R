...
windows()
	...
	windows()
	plot(nb_couples_moy, type="l", xlab='Jours', main='Nombre de couples par jour')
	
	windows()
	plot(nb_repro_moy, type='l', xlab='Jours', main='Nombre de reproductions par jour')
	
	windows()
	plot(coeff_pearson_moy, type="l", xlab="jours", main="Coefficient de correlation de Pearson entre la taille des individus appariés")
