...
tx_mort <- seq(0.001, 0.05, 0.001)

taille_pop_totale <- 1000

sex_ratio <- seq(0.4,0.6,0.1) #sex-ratio = nb males/taille pop totale
...
for ( sr_boucle in sex_ratio){
	...
	for (tm_boucle in tx_mort){
		...
		if ( instant == cpt){
			...
			write.table(pearson, file = paste("tm_test_plateau_pearson_",instant,instant_sr,".csv")) 
			instant <- instant + 24
		}
	}
	if(instant_sr == cpt_sr){
		pearson_tm_sr <- data.frame(tx_mort, pearson_tm, IC95_min_tm, IC95_max_tm)
		write.table(pearson_tm_sr, paste("pearson_tm_", instant_sr,".csv"))
		instant_sr <- instant_sr + 1
		cpt_sr <- cpt_sr + 1
	}
}
