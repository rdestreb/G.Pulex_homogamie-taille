...
taux_croissance <- seq(1.02, 1.3, 0.01)
...
IC95_max_tc <- numeric(length(taux_croissance))
IC95_min_tc <- numeric(length(taux_croissance))
tc_boucle <- 0
...
for (tc_boucle in taux_croissance){
	...
	IC95_min_tc[cpt] <- IC95_min_1000
	IC95_max_tc[cpt] <- IC95_max_1000
}
	...
	pearson <- data.frame(taux_croissance, pearson_tc, IC95_min_tc, IC95_max_tc)
	# write.table(pearson, "script21.csv")
...
datafull <- read.table("data_script21.csv")
windows()
plot(pearson_tc ~ taux_croissance, data = datafull,
		type = "l", col = "red", lwd = 3, 
		xlim = c(1.02,1.3),
		ylim = c(-0.1,0.6),
		xlab = "Taux de croissance",
		ylab = "Coefficient d'homogamie",
		bty = "l",
		cex.lab=policeaxes, cex.axis = policevaleuraxes)
abline(h=0 , lty=1)
abline(v=1.1, lty = 5 , lwd = 2)
lines(IC95_min_tc ~ taux_croissance, data = datafull, col = "black", lwd = 3, lty = 3)
lines(IC95_max_tc ~ taux_croissance, data = datafull, col = "black", lwd = 3, lty = 3)
	# windows()
	# plot(pearson_tc~taux_croissance, type="l", ylim=c(0,1), xlim=c(min(taux_croissance),max(taux_croissance)), ylab="Coefficient de Pearson", xlab="Taux de croissance", 
			# main="Valeurs d'homogamie à t = 1000 en fonction du taux de croissance",
			# sub=paste("effectif pop = ",taille_pop_totale," ; tx_mortalité = ",tx_mort," ; sex-ratio = ",sex_ratio," ; réplicats = ",repetition ))
