...
for (iteration in 1:repetition){
	...
	taille_male_couple <- numeric(tps_max)
	taille_male_seul <- numeric(tps_max)
	taille_femelle_couple <- numeric(tps_max)
	taille_femelle_seule <- numeric(tps_max)
	...
	for (tps in 1:tps_max) {
	  ...
	  # pour chaque mort, un individu nait :
		...
		if (tps = 1000) {
			m_taille_couple_moy_1000 <- mean(m_taille[m_no_femelle != 0])
			m_taille_seul_moy_1000 <- mean(m_taille[m_no_femelle == 0])
			
			f_taille_couple_moy_1000 <- mean(f_taille[f_no_male != 0])
			f_taille_seule_moy_1000 <- mean(f_taille[f_no_male == 0])
		}
		
	}
		taille_male_couple[iteration] <- m_taille_couple_moy_1000
		taille_male_seul[iteration] <- m_taille_seul_moy_1000
		
		taille_femelle_couple[iteration] <- f_taille_couple_moy_1000
		taille_femelle_seule[iteration] <- f_taille_seule_moy_1000
}
	...
	tableau <- data.frame(temps, taille_male_seul, taille_male_couple)
	taille_males <- c(taille_male_seul, taille_male_couple)
	etat_males <- c(rep("Seul",length(taille_male_seul)), rep("En couple",length(taille_male_couple)))
	tableau2 <- data.frame(taille_males, etat_males)
	
	tableau3 <- data.frame(temps, taille_femelle_seule, taille_femelle_couple)
	taille_femelles <- c(taille_femelle_seule, taille_femelle_couple)
	etat_femelles <- c(rep("Seule",length(taille_femelle_seule)), rep("En couple",length(taille_femelle_couple)))
	tableau4 <- data.frame(taille_femelles, etat_femelles)
	...
	windows()
	boxplot(tableau2$taille_males~tableau2$etat_males, ylim=c(min(taille_males)-0.05,max(taille_males)+0.05), ylab="Taille", 
	...
	windows()
	boxplot(tableau4$taille_femelles~tableau4$etat_femelles, ylim=c(min(taille_femelles)-0.05,max(taille_femelles)+0.05), ylab="Taille", 
