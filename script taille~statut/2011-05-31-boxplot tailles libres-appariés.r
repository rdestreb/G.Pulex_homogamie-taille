...
lme_taille_male <- NULL
lme_etat_male <- NULL
lme_replicat_male <- NULL

lme_taille_femelle <- NULL
lme_etat_femelle <- NULL
lme_replicat_femelle <- NULL
...
for (tour in 1:iteration){
	...
	lme_taille_male <- c(lme_taille_male,taille_male_seul_1000, taille_male_couple_1000)
	lme_etat_male <- c(lme_etat_male,rep("Seul",length(taille_male_seul_1000)), rep("En couple",length(taille_male_couple_1000)))
	lme_replicat_male <- c(lme_replicat_male,rep(tour,nm))
	tableau <- data.frame(lme_taille_male, lme_etat_male,lme_replicat_male)
	
	write.table(tableau,paste("boxplot mâles.csv"))
	
	lme_taille_femelle <- c(lme_taille_femelle,taille_femelle_seule_1000, taille_femelle_couple_1000)
	lme_etat_femelle <- c(lme_etat_femelle,rep("Seule",length(taille_femelle_seule_1000)), rep("En couple",length(taille_femelle_couple_1000)))
	lme_replicat_femelle <- c(lme_replicat_femelle,rep(tour,nf))
	tableau2 <- data.frame(lme_taille_femelle, lme_etat_femelle,lme_replicat_femelle)
	write.table(tableau2,paste("boxplot femelles.csv"))
...
m_boxplot <- read.table("boxplot mâles.csv")
summary(lme(lme_taille_male~lme_etat_male,random=~1|lme_replicat_male,data=m_boxplot))

f_boxplot <- read.table("boxplot femelles.csv")
summary(lme(lme_taille_femelle~lme_etat_femelle,random=~1|lme_replicat_femelle,data=f_boxplot))
	boxplot(lme_taille_femelle~lme_etat_femelle)
	
	# m_bp_tot <- c(m_bp_1,m_bp_2)
