	...
	for (tps in 1:tps_max) {
		...
		if ((nb_femelle_celibataire > 0) & (nb_male_celibataire > 0)){
			...
			for (ligne in no_male_celibataire){
				...
				if (length(no_femelle_celibataire) != 0){
					# /!\  "bug du sample"  /!\
					if (length(no_femelle_celibataire) == 1) {
						no_femelle_selec <- no_femelle_celibataire
					} else if (length(no_femelle_celibataire) > 1) {
						# choix aléatoire du male :
						no_femelle_selec <- sample(no_femelle_celibataire,1)
					}
