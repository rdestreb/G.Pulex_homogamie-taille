...
sex_ratio <- 0.5
intercept <- seq(1.75,9.75,1)

pearson_ic <- numeric(length(sex_ratio))
IC95_max_ic <- numeric(length(sex_ratio))
IC95_min_ic <- numeric(length(sex_ratio))
intercept_boucle <- 0
cpt <- 0  #compteur (nombre de tour de code)
instant <- 1
for (intercept_boucle in intercept){
	...
	################################
		...
		f_max_dist_mue <- round((intercept+14.83*f_taille),2)
		...
		m_max_dist_mue <- round((intercept+14.83*m_taille),2)	
	...
	################################
		...
		for (tps in 1:tps_max) {
		  ...
		  # pour chaque mort, un individu nait :
			...
			f_max_dist_mue[no_femelle_mort] <- round((intercept + 14.83 * f_taille[no_femelle_mort]),2)
			...
			m_max_dist_mue[no_male_mort] <- round((intercept + 14.83 * m_taille[no_male_mort]),2)
			...
			if (nbmuetoday > 0) {
				...
				f_max_dist_mue[femelle_en_mue] <- round((intercept+14.83*f_taille[femelle_en_mue]),2)
				f_dist_mue[femelle_en_mue] <- f_max_dist_mue[femelle_en_mue]
				m_max_dist_mue[male_en_mue] <- round((intercept+14.83*m_taille[male_en_mue]),2)
	...
	pearson_ic[cpt] <- pearson_1000
	IC95_min_ic[cpt] <- IC95_min_1000
	IC95_max_ic[cpt] <- IC95_max_1000
	
	if ( instant == cpt ){
		pearson <- data.frame(coeff_pearson_moy)
		write.table(pearson, file = paste("coeffs_pearson_",instant,".csv")) 
		instant <- instant + 10
	}
}

homogamie_ic <-data.frame(intercept, pearson_ic, IC95_min_ic, IC95_max_ic)
write.table(homogamie_ic, file = "homogamie_intercept.csv")
datafull <- read.table("homogamie_intercept.csv")
summary(datafull)
plot(pearson_ic ~ intercept, data = datafull,
		type = "l", col = "red", lwd = 3, 
		ylim = c(-0.1, 1),
		xlim = c(0,10),
		xlab = "intercept",
		ylab = "coeff homogamie", 
		sub=paste("effectif pop = ",taille_pop_totale," ; tx_croissance = ",tx_croissance," ; tx_mortalité = ",txmort_male," ; réplicats = ",repetition ))
lines(IC95_min_ic ~ intercept, data = datafull, col = "black", lwd = 1, lty = 3)
lines(IC95_max_ic ~ intercept, data = datafull, col = "black", lwd = 1, lty = 3)
fin <- Sys.time()
warnings()
fin - debut