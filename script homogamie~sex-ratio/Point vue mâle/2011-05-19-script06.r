#########################################################
####	TEST 2 : SEQUENCE COURTE, POINT DE VUE MÂLE	#####
#########################################################
...
for (sr_boucle in sex_ratio){
	...
	if ( instant == cpt ){
		...
		write.table(pearson, file = paste("script06_coeffs_pearson_",instant,".csv")) 
		instant <- instant + 10
	}
}

homogamie_sr <-data.frame(sex_ratio, pearson_sr, IC95_min_sr, IC95_max_sr)
write.table(homogamie_sr, file = "homogamie_sex_ratio06.csv")
# datafull <- read.table("homogamie_sex_ratio06.csv")
# summary(datafull)
