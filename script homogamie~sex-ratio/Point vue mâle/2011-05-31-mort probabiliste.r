...
sex_ratio <- c(seq(0.1,0.4,0.02),seq(0.405,0.6,0.005),seq(0.62,0.9,0.02)) #sex-ratio = nb males/taille pop totale
	...
	################################
		...
		for (tps in 1:tps_max) {
		  ...
		  # la mort s'abat aléatoirement sur les individus :
			no_male_mort <- m_no[txmort_male > runif(nm,0,1)]
			no_femelle_mort <- f_no[txmort_femelle > runif(nf,0,1)]
			
			nb_male_mort <- length(no_male_mort)
			nb_femelle_mort <- length(no_femelle_mort)
...
write.table(homogamie_sr, file = "script42.csv")
...
# datafull <- read.table("script42.csv")
...
# lines(IC95_min_sr ~ sex_ratio, data = datafull, col = "black", lwd = 3, lty = 3)
# lines(IC95_max_sr ~ sex_ratio, data = datafull, col = "black", lwd = 3, lty = 3)
# abline(h = 0, lty = 1)
# abline(v = 0.5 , lty = 5, lwd = 2)  
# fin <- Sys.time()
# warnings()
# fin - debut