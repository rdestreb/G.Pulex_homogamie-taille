#################################################################
####	TEST 1 : SEQUENCE COMPLETE, POINT DE VUE FEMELLE	#####
#################################################################
...
sex_ratio <- c(seq(0.1,0.4,0.02),seq(0.41,0.6,0.005),seq(0.62,0.9,0.02)) #sex-ratio = nb males/taille pop totale
# sex_ratio <- seq(0.2,0.8,0.2)
...
for (sr_boucle in sex_ratio){
	...
	if ( instant == cpt ){
		...
		write.table(pearson, file = paste("script05_coeffs_pearson_",instant,".csv")) 
		instant <- instant + 10
	}
}

homogamie_sr <-data.frame(sex_ratio, pearson_sr, IC95_min_sr, IC95_max_sr)
write.table(homogamie_sr, file = "homogamie_sex_ratio05.csv")
# datafull <- read.table("homogamie_sex_ratio05.csv")
# summary(datafull)
# plot(pearson_sr ~ sex_ratio, data = datafull,
		...
		# sub = "effectif pop = 1000 ; tx_croissance = 1.1 ; tx_mortalité = 0.012 ; réplicats = 100")
