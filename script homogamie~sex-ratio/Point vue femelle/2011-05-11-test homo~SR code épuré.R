...
sex_ratio <- seq(0.1,0.9,0.1)
pearson_sr <- numeric(length(sex_ratio))
cpt <- 0
taille_pop_tot <- 1000

for(sr in sex_ratio){ # sr <- 0.8
  
  cpt <- cpt + 1

	nm <- round(sr * taille_pop_tot)
	nf <- round(taille_pop_tot - nm)
	...
	################################
		...
		for (tps in 1:tps_max) {
			...
			if (nbmuetoday > 0) {
				...
				# qui parmi elles sont appariées (s'il y en a) ?
				if( length(f_no[(f_dist_mue <= 0) & (f_no_male != 0)]) > 0) {
				...
				if( length(m_no[(m_dist_mue <= 0) & (m_no_femelle != 0)]) > 0) {
